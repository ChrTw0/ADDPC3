...
| :---: | :---: |
| | **Negative** | **Positive** |
| **Actual** | **TN** (e.g., 8, 3, 9, 7, 2) | **FP** (e.g., 6) |
| **Negative** | | Precision (e.g., 3 out of 4) |
| **Positive** | **FN** (e.g., 5, 5) | **TP** (e.g., 5, 5, 5) |
...
*Description of Figure 3-4:* A plot with "Threshold" on the x-axis and values from 0.0 to 1.0 on the y-axis. Two curves are shown: a dashed blue line for "Precision" and a solid green line for "Recall." Precision starts low and increases steeply, while Recall starts high and decreases steeply, crossing around the 0 threshold.
...
*Description of Figure 3-5:* A plot with "Recall" on the x-axis (0.0 to 1.0) and "Precision" on the y-axis (0.0 to 1.0). The curve shows Precision starting high and dropping sharply as Recall increases, particularly after 0.8 Recall, and drops to 0 at 1.0 Recall. A red dotted line highlights a specific point on the curve.
...
*Description of Figure 3-6:* A plot with "False Positive Rate" on the x-axis (0.0 to 1.0) and "True Positive Rate (Recall)" on the y-axis (0.0 to 1.0). The curve starts at (0, 0), rises steeply to (0, 0.8) and then gradually increases toward (1, 1). A dashed line runs from (0, 0) to (1, 1). A red circle is highlighted on the curve.
...
*Description of Figure 3-7:* A plot comparing the ROC curve of the "SGD" (dashed blue line) and the "Random Forest" (solid red line). Both plot True Positive Rate (Recall) vs. False Positive Rate. The Random Forest curve is noticeably closer to the top-left corner than the SGD curve, indicating better performance.
...
*Description of Matshow:* A grayscale image plot with axes labeled 0-8. The diagonal from top-left to bottom-right is much brighter than the rest of the matrix, indicating high correct classification. The 5th row (index 5, actual 5s) and 5th column (predicted 5s) are a bit darker than others.
...
*Description of Matshow (Normalized Errors):* A grayscale image plot with axes labeled 0-8. The diagonal is now black (zero errors). Bright patches off the diagonal show high error rates. The column corresponding to class 8 is quite bright, and the boxes for confusing 3s and 5s (and vice-versa) are noticeable.
...
*Description of Digits Plot:* A 2x2 grid of 5x5 blocks of digits.
*   Top Left (X_aa): Actual 3, Predicted 3 (looks like: blocks of clear 3s)
*   Top Right (X_ab): Actual 3, Predicted 5 (looks like: blocks of 3s that look like 5s to the classifier)
*   Bottom Left (X_ba): Actual 5, Predicted 3 (looks like: blocks of 5s that look like 3s to the classifier)
*   Bottom Right (X_bb): Actual 5, Predicted 5 (looks like: blocks of clear 5s)
...
instances with that target label). To do this, simply set `average="weighted"` in the preceding code.⁴
...
3.  Tackle the Titanic dataset. A great place to start is on [Kaggle](https://www.kaggle.com).

4.  Build a spam classifier (a more challenging exercise):
    *   Download examples of spam and ham from [Apache SpamAssassin's public datasets](http://spamassassin.apache.org/publiccorpus.html).
...
Solutions to these exercises can be found in the Jupyter notebooks available at [https://github.com/ageron/handson-ml2](https://github.com/ageron/handson-ml2).
